...
import Polysemy.Process.Effect.ProcessOutput (ProcessOutput, OutputPipe (Stdout, Stderr))
...
import Polysemy.Process.Interpreter.ProcessOutput (
  ...
  interpretProcessOutputTextLines, interpretProcessOutputIgnore,
  )
...
type ScopeEffects i o e err =
  [Queue (In i), Queue (Out o), SystemProcess !! err]

scope ::
  ∀ i o e resource err r .
  ...
  ProcessOptions ->
  InterpretersFor (ScopeEffects i o e err) r
...
interpretProcess ::
  ∀ resource err i o e r .
  Member (Scoped resource (SystemProcess !! err)) r =>
  Members [ProcessOutput 'Stdout o, ProcessOutput 'Stderr o, ProcessInput i, Resource, Race, Async, Embed IO] r =>
  ProcessOptions ->
  InterpreterFor (Scoped () (Process i o e) !! ProcessError) r
interpretProcess options =
  interpretScopedResumableWith_ @(ScopeEffects i o e err) (scope @i @o @e @resource options) handleProcessWithQueues
...
interpretProcessText ::
  ...
  Members [Scoped resource (SystemProcess !! err), Resource, Race, Async, Embed IO] r =>
  ProcessOptions ->
  InterpreterFor (Scoped () (Process Text Text Text) !! ProcessError) r
